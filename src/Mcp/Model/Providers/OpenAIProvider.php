 ...
 * OpenAI Provider
 * 
 * Implements integration with OpenAI's GPT models.
...
class OpenAIProvider implements ModelProviderInterface
{
    ...
    public function __construct(array $config, ?LoggerInterface $logger = null)
    {
        ...
        if (empty($this->config['api_key'])) {
            throw new McpModelException('OpenAI API key is required');
        }
    }
    ...
    public function getSupportedModels(): array
    {
        return [
            ...
            'gpt-3.5-turbo-16k' => ['context' => 16385, 'max_tokens' => 16384],
            'gpt-4' => ['context' => 8192, 'max_tokens' => 4096],
            'gpt-4-32k' => ['context' => 32768, 'max_tokens' => 32768],
            'gpt-4-turbo-preview' => ['context' => 128000, 'max_tokens' => 4096],
            'gpt-4o' => ['context' => 128000, 'max_tokens' => 4096],
            'gpt-4o-mini' => ['context' => 128000, 'max_tokens' => 16384]
        ];
    }
    ...
    public function chat(array $messages, array $options = []): CompletionResponse
    {
        ...
        try {
            ...
            $this->stats['errors']++;
            throw new McpModelException("OpenAI API request failed: {$e->getMessage()}", 0, $e);
        }
    }
    ...
    public function streamChat(array $messages, array $options = []): \Generator
    {
        ...
        try {
            ...
            $this->stats['errors']++;
            throw new McpModelException("OpenAI stream request failed: {$e->getMessage()}", 0, $e);
        }
    }
    ...
    {
        // OpenAI token counting approximation
        // This is a rough estimate; for accurate counting, use OpenAI's tiktoken
    ...
    {
        try {
            $this->complete('Hello', ['max_tokens' => 5]);
            return true;
        } catch (\Exception $e) {
            $this->logger->warning('OpenAI connection test failed', [
                'error' => $e->getMessage()
            ]);
            return false;
        }
    ...
    private function makeRequest(string $endpoint, array $data): array
    {
        ...
        if (isset($decodedResponse['error'])) {
            throw new McpModelException(
                "OpenAI API error: " . $decodedResponse['error']['message'],
